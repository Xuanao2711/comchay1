...
class Activation {
	...
	public function process_form() {
		...
		// Kiểm tra xem biến $code, $data và $dev đã được khai báo và gán giá trị trước khi sử dụng
		$code = isset( $_POST['code'] ) ? $_POST['code'] : '';
		$data = isset( $_POST['data'] ) ? $_POST['data'] : array();
		$dev = isset( $_POST['dev'] ) ? $_POST['dev'] : '';
	
		// Kiểm tra xem mảng $data có chứa khóa 'token' hay không trước khi truy cập vào nó
		$token = isset( $data['token'] ) ? $data['token'] : '';
	
		// Thực hiện kích hoạt với các biến đã được xác định
		$this->activate( $code, $token, $dev );
